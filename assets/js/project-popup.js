...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (closePopupBtn) {
    closePopupBtn.addEventListener('click', function() {
      ...
      if (window.self === window.top) {
        hidePopup(function() {
          // Get the saved filter category, if any
          const savedCategory = localStorage.getItem('activeFilterCategory');
          
          // Redirect to portfolio section with filter parameter if available
          if (savedCategory && savedCategory !== 'all') {
            window.location.href = '../index.html#portfolio?filter=' + savedCategory;
          } else {
